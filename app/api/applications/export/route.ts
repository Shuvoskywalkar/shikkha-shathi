...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const row of rows) {
      const excelRow = sheet.addRow({ id: row.id, name: row.name, phone: row.phone, email: row.email || '', college: row.college, garden: row.garden, guardianJob: row.guardianJob, gpa: row.gpa, futurePlan: row.futurePlan, department: row.department, calculator: row.bookWriters?.__calculator === 'yes' ? 'হ্যাঁ' : 'না', geometryBox: row.bookWriters?.__geometryBox === 'yes' ? 'হ্যাঁ' : 'না', books: row.books.map((book) => row.bookWriters?.[book] ? `${book} — ${row.bookWriters[book]}` : book).join(', '), createdAt: row.createdAt.toLocaleString('bn-BD') })
      excelRow.height = 28
