...
export async function GET(request: NextRequest) {
  ...
  const sheet = XLSX.utils.json_to_sheet(rows.map((row) => ({ রেফারেন্স: row.id, নাম: row.name, মোবাইল: row.phone, কলেজ: row.college, 'চা-বাগান': row.garden, 'অভিভাবকের পেশা': row.guardianJob, GPA: row.gpa, বিভাগ: row.department, বই: row.books.join(', '), জমাদানের_সময়: row.createdAt.toISOString(), মার্কশীট: row.marksheetPath ? 'সংযুক্ত' : 'নেই', মার্কশীট_ফাইল: row.marksheetPath ? row.marksheetPath.split('/').pop() : '', প্রমাণপত্র: row.proofPath ? 'সংযুক্ত' : 'নেই', প্রমাণপত্র_ফাইল: row.proofPath ? row.proofPath.split('/').pop() : '' })))
