...
async function renderBengaliSvg(body: string, fontData: Buffer) {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1240" height="1754"><defs><style>@font-face{font-family:NotoBengali;src:url(data:font/woff;base64,${fontData.toString('base64')})}text{font-family:NotoBengali,Arial,sans-serif}</style></defs>${body}</svg>`
  return Buffer.from(new Resvg(svg, { fitTo: { mode: 'width', value: 1240 } }).render().asPng())
}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let y = 425; let detailSvg = ''
    for (const [label, value] of details) { detailSvg += `<text x="110" y="${y}" font-size="26" fill="${muted}">${esc(label)}:</text>`; wrap(value || 'দেওয়া হয়নি', 38).slice(0, 2).forEach((line, i) => { detailSvg += `<text x="390" y="${y - i * 34}" font-size="26" fill="${ink}">${esc(line)}</text>` }); y -= 58 }
    const bookLines = wrap(writerText || 'কোনো বই নির্বাচন করা হয়নি', 54).slice(0, 5).map((line, i) => `<text x="110" y="${690 + i * 34}" font-size="23" fill="${ink}">${esc(line)}</text>`).join('')
    const body = `<rect width="1240" height="1754" fill="#ffffff"/><rect width="1240" height="235" fill="${green}"/><text x="110" y="110" font-size="52" fill="#ffffff">চা-বাগান শিক্ষা সহায়তা</text><text x="110" y="170" font-size="27" fill="#f7f1e3">শিক্ষার্থী আবেদনপত্র · পূর্ণাঙ্গ প্রতিবেদন</text><text x="110" y="310" font-size="42" fill="${ink}">${esc(row.name)}</text><text x="110" y="355" font-size="22" fill="${muted}">রেফারেন্স: ${esc(id)}</text><rect x="75" y="390" width="1090" height="570" rx="18" fill="${pale}" stroke="#c7dbd0" stroke-width="3"/><text x="110" y="445" font-size="31" fill="${ink}">আবেদনকারীর তথ্য</text>${detailSvg}<text x="110" y="665" font-size="30" fill="${ink}">বই ও লেখক/প্রকাশনী</text>${bookLines}<text x="110" y="930" font-size="27" fill="${ink}">সংযুক্ত নথি</text><text x="110" y="975" font-size="23" fill="${muted}">${marksheet ? 'মার্কশীট সংযুক্ত আছে' : 'মার্কশীট পাওয়া যায়নি'} · ${proof ? 'প্রমাণপত্র সংযুক্ত আছে' : 'প্রমাণপত্র পাওয়া যায়নি'}</text>`
    const firstPage = pdf.addPage([595, 842]); const png = await renderBengaliSvg(body, fontData); const image = await pdf.embedPng(png); firstPage.drawImage(image, { x: 0, y: 0, width: 595, height: 842 })
    const addImagePage = async (title: string, buffer: Buffer | null) => { if (!buffer) return; const page = pdf.addPage([595, 842]); const embedded = buffer[0] === 0xff && buffer[1] === 0xd8 ? await pdf.embedJpg(buffer) : await pdf.embedPng(buffer); const scale = Math.min(520 / embedded.width, 690 / embedded.height, 1); page.drawImage(embedded, { x: (595 - embedded.width * scale) / 2, y: 75, width: embedded.width * scale, height: embedded.height * scale }); const label = await renderBengaliSvg(`<rect width="1240" height="1754" fill="#1a5940"/><text x="110" y="110" font-size="38" fill="#ffffff">চা-বাগান শিক্ষা সহায়তা · ${esc(title)}</text>`, fontData); const labelImage = await pdf.embedPng(label); page.drawImage(labelImage, { x: 0, y: 765, width: 595, height: 77 }) }
