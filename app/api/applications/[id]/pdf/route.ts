...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const details: [string, string][] = [['মোবাইল', text(row.phone)], ['ইমেইল', text(row.email)], ['কলেজ', text(row.college)], ['চা-বাগান', text(row.garden)], ['অভিভাবকের পেশা', text(row.guardianJob)], ['ভবিষ্যৎ পরিকল্পনা', text(row.futurePlan)], ['বিভাগ', text(row.department)]]
    ...
    await addImagePage('কলেজে ভর্তির কাগজ', proof)
