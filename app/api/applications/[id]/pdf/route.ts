...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const details: [string, string][] = [['মোবাইল', text(row.phone)], ['ইমেইল', text(row.email)], ['কলেজ', text(row.college)], ['চা-বাগান', text(row.garden)], ['অভিভাবকের পেশা', text(row.guardianJob)], ['SSC GPA', text(row.gpa)], ['ভবিষ্যৎ পরিকল্পনা', text(row.futurePlan)], ['বিভাগ', text(row.department)], ['ক্যালকুলেটর', row.bookWriters?.__calculator === 'yes' ? 'প্রয়োজন' : row.bookWriters?.__calculator === 'no' ? 'প্রয়োজন নেই' : 'উল্লেখ করা হয়নি'], ['জ্যামিতি বক্স', row.bookWriters?.__geometryBox === 'yes' ? 'প্রয়োজন' : row.bookWriters?.__geometryBox === 'no' ? 'প্রয়োজন নেই' : 'উল্লেখ করা হয়নি']]
