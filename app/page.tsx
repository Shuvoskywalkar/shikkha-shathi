...
export default function Page() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      payload.set('department', dept); payload.set('books', JSON.stringify(selected)); payload.set('bookWriters', JSON.stringify(Object.fromEntries(selected.map((book) => [book, writers[book] === 'custom' ? (customWriters[book] || 'নিজের পছন্দ') : (writers[book] || writersByBook[book] || '')]))))
  ...
  const reset = () => { setSubmitted(null); setDept(''); setSelected([]); setWriters({}); setCustomWriters({}); setFiles({ marksheet: null, proof: null }); setError(''); setFieldErrors({}) }
    ...
    <div className="wrap">
      <section className="card" id="form-card">
        ...
        {submitted ? <div className="confirm"><div className="confirm-badge"><Check size={32} /></div><h2>আবেদন সফলভাবে জমা হয়েছে</h2><p>তোমার তথ্য যাচাই করে আঞ্চলিক প্রতিনিধিরা প্রয়োজনে তোমার সাথে ফোনে যোগাযোগ করবেন। ধন্যবাদ!</p><div className="ref">{submitted.id}</div><button className="primary" onClick={reset}>আরেকটি আবেদন জমা দাও</button></div> : <form onSubmit={submit} noValidate>
          ...
          <section className="section"><SectionHead num="৩" title="বই নির্বাচন" /><p className="section-sub">তোমার বিভাগের সিলেবাস অনুযায়ী তালিকা থেকে সর্বোচ্চ ১০টি বই বেছে নাও</p>{!dept ? <div className="dept-empty">প্রথমে উপরে থেকে তোমার বিভাগ নির্বাচন করো — তারপর এখানে বইয়ের তালিকা দেখা যাবে।</div> : <><div className="book-progress"><div className="book-progress-track"><div className="book-progress-fill" style={{ width: `${selected.length * 10}%` }} /></div><span>{bn(selected.length)} / ১০ নির্বাচিত</span></div><div className="book-grid">{books[dept].map((book) => <div key={book} className={`book-item ${selected.includes(book) ? 'checked' : ''} ${!selected.includes(book) && selected.length >= 10 ? 'disabled' : ''}`}><label><input type="checkbox" checked={selected.includes(book)} onChange={() => toggleBook(book)} /><span><b>{book}</b>{writersByBook[book] && <small className="book-writer">লেখক: {writersByBook[book]}</small>}</span></label>{selected.includes(book) && (publisherOptions[book] ? <select className="writer-input" value={writers[book] || ''} onChange={(event) => setWriters((value) => ({ ...value, [book]: event.target.value }))} aria-label={`${book} লেখক বা প্রকাশনী`}><option value="">লেখক / প্রকাশনী বেছে নাও</option>{publisherOptions[book].map((option) => <option key={option} value={option}>{option}</option>)}<option value="custom">নিজে লিখব</option></select> : <span className="book-writer">লেখক: NCTB</span>)}{selected.includes(book) && writers[book] === 'custom' && <input className="writer-input" value={customWriters[book] || ''} onChange={(event) => setCustomWriters((value) => ({ ...value, [book]: event.target.value }))} placeholder="নিজের পছন্দের লেখক / প্রকাশনী লিখুন" aria-label={`${book} নিজের পছন্দ`} />}</div>)}</div></>}</section>
