...
import { ArrowDown, Check, ChevronDown, Leaf, LockKeyhole, Upload, X } from 'lucide-react'
...
const books: Record<Department, string[]> = {
  science: ['বাংলা ২য় পত্র', 'ইংরেজি ২য় পত্র', 'পদার্থবিজ্ঞান ১ম পত্র', 'পদার্থবিজ্ঞান ২য় পত্র', 'রসায়ন ১ম পত্র', 'রসায়ন ২য় পত্র', 'জীববিজ্ঞান ১ম পত্র', 'জীববিজ্ঞান ২য় পত্র', 'উচ্চতর গণিত ১ম পত্র', 'উচ্চতর গণিত ২য় পত্র'],
  arts: ['বাংলা ২য় পত্র', 'ইংরেজি ২য় পত্র', 'ইতিহাস ১ম পত্র', 'ইতিহাস ২য় পত্র', 'পৌরনীতি ও সুশাসন ১ম পত্র', 'পৌরনীতি ও সুশাসন ২য় পত্র', 'অর্থনীতি ১ম পত্র', 'অর্থনীতি ২য় পত্র', 'ভূগোল ১ম পত্র', 'ভূগোল ২য় পত্র', 'সমাজকর্ম ১ম পত্র', 'যুক্তিবিদ্যা ১ম পত্র'],
  commerce: ['বাংলা ২য় পত্র', 'ইংরেজি ২য় পত্র', 'হিসাববিজ্ঞান ১ম পত্র', 'হিসাববিজ্ঞান ২য় পত্র', 'ব্যবসায় সংগঠন ও ব্যবস্থাপনা ১ম পত্র', 'ব্যবসায় সংগঠন ও ব্যবস্থাপনা ২য় পত্র', 'ফিনান্স, ব্যাংকিং ও বিমা ১ম পত্র', 'ফিনান্স, ব্যাংকিং ও বিমা ২য় পত্র', 'উৎপাদন ব্যবস্থাপনা ও বিপণন ১ম পত্র'],
}

const deptLabels: Record<Department, string> = { science: 'বিজ্ঞান বিভাগ', arts: 'মানবিক বিভাগ', commerce: 'ব্যবসায় শিক্ষা বিভাগ' }
const nctbBooks = new Set(['বাংলা ১ম পত্র', 'ইংরেজি ১ম পত্র', 'তথ্য ও যোগাযোগ প্রযুক্তি'])
const writersByBook: Record<string, string> = {
  'বাংলা ২য় পত্র': 'হায়াৎ সাইফ', 'ইংরেজি ২য় পত্র': 'M. A. Hamid', 'পদার্থবিজ্ঞান ১ম পত্র': 'এস. আর. খান', 'পদার্থবিজ্ঞান ২য় পত্র': 'এস. আর. খান', 'রসায়ন ১ম পত্র': 'হাজারী ও নাগ', 'রসায়ন ২য় পত্র': 'হাজারী ও নাগ', 'জীববিজ্ঞান ১ম পত্র': 'আবুল হাসান', 'জীববিজ্ঞান ২য় পত্র': 'আবুল হাসান', 'উচ্চতর গণিত ১ম পত্র': 'কে. বি. শাহ', 'উচ্চতর গণিত ২য় পত্র': 'কে. বি. শাহ', 'ইতিহাস ১ম পত্র': 'এ. কে. এম. শাহনাওয়াজ', 'ইতিহাস ২য় পত্র': 'এ. কে. এম. শাহনাওয়াজ', 'পৌরনীতি ও সুশাসন ১ম পত্র': 'মো. মোজাম্মেল হক', 'পৌরনীতি ও সুশাসন ২য় পত্র': 'মো. মোজাম্মেল হক', 'অর্থনীতি ১ম পত্র': 'ড. মো. শামসুল আলম', 'অর্থনীতি ২য় পত্র': 'ড. মো. শামসুল আলম', 'ভূগোল ১ম পত্র': 'ড. হুমায়ুন কবির', 'ভূগোল ২য় পত্র': 'ড. হুমায়ুন কবির', 'সমাজকর্ম ১ম পত্র': 'মো. শহীদুজ্জামান', 'যুক্তিবিদ্যা ১ম পত্র': 'ড. মো. আবদুর রাজ্জাক', 'হিসাববিজ্ঞান ১ম পত্র': 'মো. জাকির হোসেন', 'হিসাববিজ্ঞান ২য় পত্র': 'মো. জাকির হোসেন', 'ব্যবসায় সংগঠন ও ব্যবস্থাপনা ১ম পত্র': 'মো. সাইফুল ইসলাম', 'ব্যবসায় সংগঠন ও ব্যবস্থাপনা ২য় পত্র': 'মো. সাইফুল ইসলাম', 'ফিনান্স, ব্যাংকিং ও বিমা ১ম পত্র': 'মো. আবদুল্লাহ আল মামুন', 'ফিনান্স, ব্যাংকিং ও বিমা ২য় পত্র': 'মো. আবদুল্লাহ আল মামুন', 'উৎপাদন ব্যবস্থাপনা ও বিপণন ১ম পত্র': 'মো. জাহাঙ্গীর আলম'
}
...
export default function Page() {
  ...
  const [supplyNeeds, setSupplyNeeds] = useState({ calculator: 'no', geometryBox: 'no' })
  const [files, setFiles] = useState<{ admission: FileState; guardianProof: FileState }>({ admission: null, guardianProof: null })
  ...
  const downloadAdminFile = async (url: string, filename: string) => {
    ...
    try {
      const response = await fetch(url, { headers: { 'x-admin-pass': 'ShinzoWoSassageyo2026' }, cache: 'no-store' })
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const gpa = Number(data.get('gpa')); if (!Number.isFinite(gpa) || gpa < 1 || gpa > 5) errors.gpa = 'GPA ১ থেকে ৫-এর মধ্যে দিন'
  ...
  if (files.guardianProof?.error || !files.guardianProof?.dataUrl) errors.guardianProof = files.guardianProof?.error || 'অভিভাবকের কাগজ আপলোড করুন'
    setFieldErrors(errors)
    if (Object.keys(errors).length) { const firstError = Object.keys(errors)[0]; setError(`অনুগ্রহ করে ${Object.keys(errors).length}টি বিষয় ঠিক করুন`); window.requestAnimationFrame(() => { document.getElementById('form-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); window.setTimeout(() => document.querySelector<HTMLElement>(`[name="${firstError}"]`)?.focus(), 350) }); return }
    setError(''); setSubmitting(true)
    try {
      const payload = new FormData(); for (const [key, value] of data.entries()) payload.append(key, value)
      payload.set('department', dept); payload.set('books', JSON.stringify(selected)); payload.set('bookWriters', JSON.stringify({ ...Object.fromEntries(selected.filter((book) => !nctbBooks.has(book)).map((book) => [book, customWriters[book] || ''])), __calculator: dept === 'science' ? (supplyNeeds.calculator || 'no') : 'no', __geometryBox: ['science', 'commerce'].includes(dept) ? (supplyNeeds.geometryBox || 'no') : 'no' }))
      if (files.admission?.dataUrl) payload.set('admission', await (await fetch(files.admission.dataUrl)).blob(), files.admission.name)
      if (files.guardianProof?.dataUrl) payload.set('guardianProof', await (await fetch(files.guardianProof.dataUrl)).blob(), files.guardianProof.name)
      const response = await fetch('/api/applications', { method: 'POST', body: payload }); const result = await readResponse(response)
      if (!response.ok) { setFieldErrors(result.fields || {}); setError(result.error || 'আবেদন জমা দেওয়া যায়নি'); window.requestAnimationFrame(() => document.getElementById('form-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' })); return }
  ...
  const reset = () => { setSubmitted(null); setDept(''); setSelected([]); setCustomWriters({}); setSupplyNeeds({ calculator: 'no', geometryBox: 'no' }); setFiles({ admission: null, guardianProof: null }); setError(''); setFieldErrors({}) }
  ...
  return <main>
    ...
    <section className="hero">
      <div className="hero-inner">
        <p className="eyebrow">আয়োজনে: চা-বাগানের বিশ্ববিদ্যালয় পড়ুয়া শিক্ষার্থীবৃন্দ ও শিক্ষা সচেতন যুবসমাজ</p>
        ...
        <p className="hero-description">এসএসসি পরীক্ষায় পাশ করে একাদশ শ্রেণিতে ভর্তি হতে যাচ্ছ? তথ্য যাচাই, বাছাই ও পরিশেষে মৌখিক পরীক্ষার মাধ্যমে যোগ্য শিক্ষার্থীদের চূড়ান্তভাবে নির্বাচন করা হবে এবং প্রয়োজনীয় শিক্ষা উপকরণসমূহ কিনে দেওয়া হবে। আবেদনকারীর পারিবারিক আর্থিক অবস্থা, ভাইভার ফলাফল ও উচ্চশিক্ষার প্রতি আগ্রহ—এসব বিষয় বিবেচনায় নেওয়া হবে।</p>
      </div>
    ...
    <div className="wrap">
      <section className="card" id="form-card">
        ...
        {submitted ? <div className="confirm" id="confirmation-message" tabIndex={-1} role="status" aria-live="polite"><div className="confirm-badge"><Check size={32} /></div><h2>আবেদন সফলভাবে জমা হয়েছে</h2><p>তোমার তথ্য যাচাই করে আঞ্চলিক প্রতিনিধিরা প্রয়োজনে তোমার সাথে ফোনে যোগাযোগ করবেন। ধন্যবাদ!</p><div className="ref">{submitted.id}</div><button className="primary" onClick={reset}>আরেকটি আবেদন জমা দাও</button></div> : <form onSubmit={submit} noValidate>
          ...
          <section className="section"><SectionHead num="২" title="শিক্ষা সংক্রান্ত তথ্য" /><div className="field"><label>আবেদনকারীর কলেজের নাম <span className="req">*</span></label><input name="college" placeholder="কলেজের পূর্ণ নাম" /></div><div className="grid2"><div className="field"><label>SSC পরীক্ষার ফলাফল (GPA) <span className="req">*</span></label><input name="gpa" type="number" min="1" max="5" step=".01" placeholder="যেমনঃ 4.50" /></div><div className="field"><label>আবেদনকারী কলেজে কোন বিভাগে অধ্যয়নরত? <span className="req">*</span></label><select value={dept} onChange={(e) => { setDept(e.target.value as Department); setSelected([]); setSupplyNeeds({ calculator: 'no', geometryBox: 'no' }) }}><option value="">বিভাগ নির্বাচন করুন</option><option value="science">বিজ্ঞান বিভাগ</option><option value="arts">মানবিক বিভাগ</option><option value="commerce">ব্যবসায় শিক্ষা বিভাগ</option></select></div></div></section>
          <section className="section"><SectionHead num="৩" title="ভবিষ্যৎ পরিকল্পনা" /><div className="field"><label>বড় হয়ে কী হতে চাও <span className="req">*</span></label><textarea name="futurePlan" rows={7} placeholder="সংক্ষেপে লিখো— উচ্চমাধ্যমিক পাশ করার পরবর্তী পরিকল্পনা কী তোমার? এইচএসসি শেষে কী করতে চাও?" /></div></section>
          <section className="section"><SectionHead num="৪" title="বই নির্বাচন" /><p className="section-sub">তোমার বিভাগের সিলেবাস অনুযায়ী তালিকা থেকে সর্বোচ্চ ১০টি বই বেছে নাও। বাংলা ১ম পত্র, ইংরেজি ১ম পত্র ও তথ্য ও যোগাযোগ প্রযুক্তি (আইসিটি) এই সহায়তার তালিকায় রাখা হয়নি—এগুলো NCTB-এর অপরিবর্তনীয় বই এবং সাধারণত সিনিয়র শিক্ষার্থীদের কাছ থেকে বিনামূল্যে সংগ্রহ করা যায়।</p>{!dept ? <div className="dept-empty">প্রথমে উপরে থেকে তোমার বিভাগ নির্বাচন করো — তারপর এখানে বইয়ের তালিকা দেখা যাবে।</div> : <><div className="book-progress"><div className="book-progress-track"><div className="book-progress-fill" style={{ width: `${selected.length * 10}%` }} /></div><span>{bn(selected.length)} / ১০ নির্বাচিত</span></div><div className="book-grid">{books[dept].map((book) => <div key={book} className={`book-item ${selected.includes(book) ? 'checked' : ''} ${!selected.includes(book) && selected.length >= 10 ? 'disabled' : ''}`}><label><input type="checkbox" checked={selected.includes(book)} onChange={() => toggleBook(book)} /><span><b>{book}</b></span></label>{selected.includes(book) && !nctbBooks.has(book) && <input className="writer-input" value={customWriters[book] || ''} onChange={(event) => setCustomWriters((value) => ({ ...value, [book]: event.target.value }))} placeholder="লেখক বা প্রকাশনীর নাম লিখুন" aria-label={`${book} লেখক বা প্রকাশনীর নাম`} />}</div>)}</div></>}</section>
          {(dept === 'science' || dept === 'commerce') && <section className="section"><SectionHead num="৫" title="অতিরিক্ত শিক্ষা উপকরণ" /><p className="section-sub">বিজ্ঞান বিভাগের জন্য বৈজ্ঞানিক ক্যালকুলেটর এবং বিজ্ঞান ও ব্যবসায় শিক্ষা বিভাগের জন্য জ্যামিতি বক্সের প্রয়োজন জানাও।</p><div className="supply-checks">{dept === 'science' && <label className="check-option"><input type="checkbox" checked={supplyNeeds.calculator === 'yes'} onChange={(event) => setSupplyNeeds((current) => ({ ...current, calculator: event.target.checked ? 'yes' : 'no' }))} /> <span>বৈজ্ঞানিক ক্যালকুলেটর প্রয়োজন</span></label>}<label className="check-option"><input type="checkbox" checked={supplyNeeds.geometryBox === 'yes'} onChange={(event) => setSupplyNeeds((current) => ({ ...current, geometryBox: event.target.checked ? 'yes' : 'no' }))} /> <span>জ্যামিতি বক্স প্রয়োজন</span></label></div></section>}<section className="section"><SectionHead num={dept === 'science' || dept === 'commerce' ? '৬' : '৫'} title="প্রয়োজনীয় কাগজপত্র" /><FilePicker label="কলেজে ভর্তির কাগজ আপলোড করো" value={files.admission} error={fieldErrors.admission} onChange={(value) => setFiles((current) => ({ ...current, admission: value }))} /><FilePicker label="অভিভাবকের পেশার প্রমাণপত্র / চা-বাগানের অধিবাসী প্রত্যয়নপত্র আপলোড করো" value={files.guardianProof} error={fieldErrors.guardianProof} onChange={(value) => setFiles((current) => ({ ...current, guardianProof: value }))} /></section>
      ...
      <section className="future-plan" aria-labelledby="future-plan-title"><div className="future-plan-header"><span className="future-plan-kicker">ভবিষ্যৎ সম্ভাব্য কর্মসূচি</span><h2 id="future-plan-title">সকল আবেদনকারীর জন্য ভবিষ্যৎ পরিকল্পনা</h2><p>বই-সংক্রান্ত সহায়তার জন্য কোনো শিক্ষার্থী নির্বাচিত হোক বা না হোক, প্রত্যেক আবেদন�����া���ীকে একাদশ–দ্বাদশ শ্রেণিকালে শিক্ষা সহায়তা ও উন্নয়নমূলক কার্যক্রমের আওতায় রাখার সম্ভাবনা নিয়ে আলোচনা চলছে।</p></div><div className="future-plan-grid"><article><span className="future-plan-number">০১</span><h3>নিয়মিত দিকনির্দেশনা</h3><p>শিক্ষার্থীদের নিয়মিত চর্চা ও তত্ত্বাবধানে রাখার জন্য ধারাবাহিক দিকনির্দেশনা।</p></article><article className="future-plan-featured"><span className="future-plan-number">০২</span><h3>মাসিক ক্লাস ও পরীক্ষা</h3><p>প্রতি মাসে চার সপ্তাহের মধ্যে প্রথম তিন সপ্তাহে বাংলা, ইংরেজি ও সাধারণ জ্ঞানের একটি করে ক্লাস এবং শেষ সপ্তাহে একটি পরীক্ষা নেওয়ার পরিকল্পনা রয়েছে। এর মাধ্যমে শিক্ষার্থীদের নিয়মিত চর্চা ও তত্ত্বাবধান নিশ্চিত করা হবে।</p></article><article><span className="future-plan-number">০৩</span><h3>ফলাফলভিত্তিক সহায়তা</h3><p>শিক্ষার্থীর অগ্রগতি ও ফলাফলের ভিত্তিতে সম্ভাব্য আর্থিক সহযোগিতা দেওয়ার পরিকল্পনা রয়েছে।</p></article></div><div className="future-plan-goal"><strong>লক্ষ্য</strong><span>চা-বাগান অঞ্চলের শিক্ষার্থীদের উচ্চশিক্ষার পথে এগিয়ে যেতে উৎসাহ, দিকনির্দেশনা ও সহায়তা প্রদান করে বিশ্ববিদ্যালয় ভর্তি পরীক্ষায় অকৃতকার্য হওয়ার হার কমিয়ে আনা।</span></div></section><footer><button className="admin-link" aria-label="প্রশাসন প্যানেল" onClick={() => setShowAdmin((value) => !value)}><LockKeyhole size={16} /></button></footer>
      {showAdmin && <section className="card admin-panel">{!adminUnlocked ? <div className="admin-login"><h2>প্রশাসন প্যানেল</h2><input id="admin-pass" type="password" placeholder="পাসকোড দিন" /><button className="primary" onClick={unlockAdmin}>প্রবেশ করুন</button>{adminAction && <div className="admin-progress" role="status" aria-live="polite"><span className="submit-spinner" aria-hidden="true" />{adminAction}</div>}{adminError && <div className="err-box" role="alert">{adminError}</div>}</div> : <div><div className="admin-heading"><h2>আবেদনসমূহ ({applications.length})</h2>{adminAction && <div className="admin-progress" role="status" aria-live="polite"><span className="submit-spinner" aria-hidden="true" />{adminAction}</div>}<button className="export-btn" disabled={Boolean(adminAction)} onClick={async () => { setAdminError(''); setAdminAction('Excel তৈরি হচ্ছে...'); try { const response = await fetch('/api/applications/export', { headers: { 'x-admin-pass': 'ShinzoWoSassageyo2026' }, cache: 'no-store' }); if (!response.ok) { const result = await readResponse(response); setAdminError(result.error || `Excel তৈরি করা যায়নি (${response.status})`); return } const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'tea-garden-applications.xlsx'; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000) } catch { setAdminError('সার্ভারের সাথে যোগাযোগ করা যায়নি') } finally { setAdminAction('') } }}>সব আবেদন Excel</button></div>{adminError && <div className="err-box" role="alert">{adminError}</div>}{applications.length === 0 ? <p className="hint">এখনও কোনো আবেদন নেই।</p> : applications.map((app) => <details className="app-card" key={app.id}><summary>{app.name}<span>{deptLabels[app.dept]} · {new Date(app.submittedAt).toLocaleDateString('bn-BD')} <ChevronDown size={15} /></span></summary><p>ফোন: {app.phone}<br />কলেজ: {app.college}<br />GPA: {app.gpa}<br />ক্যালকুলেটর: {app.calculatorNeed === 'yes' ? 'প্রয়োজন' : app.calculatorNeed === 'no' ? 'প্রয়োজন নেই' : 'উল্লেখ করা হয়নি'}<br />জ্যামিতি বক্স: {app.geometryBoxNeed === 'yes' ? 'প্রয়োজন' : app.geometryBoxNeed === 'no' ? 'প্রয়োজন নেই' : 'উল্লেখ করা হয়নি'}<br />বড় হয়ে কী হতে চাও: {app.futurePlan}<br />চা-বাগান: {app.garden}<br />অভিভাবকের পেশা: {app.guardianJob}<br />GPA: {app.gpa}<br />বই: {app.books.join(', ')}<br />মার্কশীট: {app.marksheet?.name || 'নেই'}<br />প্রমাণপত্র: {app.proof?.name || 'নেই'}</p><div className="document-links"><button type="button" disabled={Boolean(adminAction)} onClick={() => openAdminFile(`/api/applications/${app.id}/files/marksheet`, `${app.id}-marksheet`)}>মার্কশীট দেখুন</button><button type="button" disabled={Boolean(adminAction)} onClick={() => openAdminFile(`/api/applications/${app.id}/files/proof`, `${app.id}-proof`)}>প্রমাণপত্র দেখুন</button><button type="button" disabled={Boolean(adminAction)} onClick={() => downloadAdminFile(`/api/applications/${app.id}/pdf`, `${app.id}.pdf`)}>PDF ডাউনলোড</button></div></details>)}</div>}</section>}
    </div>
  </main>
}
