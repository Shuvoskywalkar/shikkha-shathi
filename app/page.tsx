...
export default function Page() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitted(result.application); setApplications((current) => [result.application, ...current]); window.setTimeout(() => document.getElementById('confirmation-message')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0)
