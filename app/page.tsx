...
export default function Page() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!supplyNeeds.calculator) errors.calculator = 'ক্যালকুলেটর প্রয়োজন কি না নির্বাচন করুন'
    if (!supplyNeeds.geometryBox) errors.geometryBox = 'জ্যামিতি বক্স প্রয়োজন কি না নির্বাচন করুন'
    ...
    try {
      ...
      payload.set('department', dept); payload.set('books', JSON.stringify(selected)); payload.set('bookWriters', JSON.stringify({ ...Object.fromEntries(selected.filter((book) => !nctbBooks.has(book)).map((book) => [book, customWriters[book] || ''])), __calculator: supplyNeeds.calculator, __geometryBox: supplyNeeds.geometryBox }))
  ...
  return <main>
    ...
    <section className="hero">
      <div className="hero-inner">
        ...
        <p className="hero-description">এসএসসি পাশ করে একাদশ শ্রেণিতে ভর্তি হতে যাচ্ছ? তথ্য যাচাই, বাছাই ও প্রয়োজনে ভাইভার মাধ্যমে চূড়ান্তভাবে নির্বাচিত শিক্ষার্থীদের প্রয়োজনীয় বই, ক্যালকুলেটর ও জ্যামিতি বক্স দিয়ে সহায়তা করা হবে।</p><p className="hero-note">পারিবারিক আর্থিক অবস্থা, ভাইভার ফলাফল ও উচ্চশিক্ষার প্রতি আগ্রহ—এই তিনটি বিষয় বিবেচনা করে যোগ্য শিক্ষার্থীদের চূড়ান্তভাবে নির্বাচন করা হবে। নির্বাচিতদের কাঙ্ক্ষিত পাঠ্যবই কিনে দেওয়া হবে।</p>
      </div>
    ...
    <div className="wrap">
      <section className="card" id="form-card">
        ...
        {submitted ? <div className="confirm" id="confirmation-message" tabIndex={-1} role="status" aria-live="polite"><div className="confirm-badge"><Check size={32} /></div><h2>আবেদন সফলভাবে জমা হয়েছে</h2><p>তোমার তথ্য যাচাই করে আঞ্চলিক প্রতিনিধিরা প্রয়োজনে তোমার সাথে ফোনে যোগাযোগ করবেন। ধন্যবাদ!</p><div className="ref">{submitted.id}</div><button className="primary" onClick={reset}>আরেকটি আবেদন জমা দাও</button></div> : <form onSubmit={submit} noValidate>
          ...
          <section className="section"><SectionHead num="৪" title="অতিরিক্ত শিক্ষা উপকরণ" /><p className="section-sub">ক্যালকুলেটর ও জ্যামিতি বক্সের প্রয়োজন আছে কি না জানাও।</p><div className="grid2"><div className="field"><label>ক্যালকুলেটর প্রয়োজন?</label><select name="calculator" value={supplyNeeds.calculator} aria-invalid={Boolean(fieldErrors.calculator)} onChange={(event) => setSupplyNeeds((current) => ({ ...current, calculator: event.target.value }))}><option value="">নির্বাচন করুন</option><option value="yes">হ্যাঁ, প্রয়োজন</option><option value="no">না, প্রয়োজন নেই</option></select></div><div className="field"><label>জ্যামিতি বক্স প্রয়োজন?</label><select name="geometryBox" value={supplyNeeds.geometryBox} aria-invalid={Boolean(fieldErrors.geometryBox)} onChange={(event) => setSupplyNeeds((current) => ({ ...current, geometryBox: event.target.value }))}><option value="">নির্বাচন করুন</option><option value="yes">হ্যাঁ, প্রয়োজন</option><option value="no">না, প্রয়োজন নেই</option></select></div></div></section><section className="section"><SectionHead num="৫" title="প্রয়োজনীয় কাগজপত্র" /><FilePicker label="নিজের SSC পরীক্ষার মার্কশীট আপলোড করো" value={files.marksheet} error={fieldErrors.marksheet} onChange={(value) => setFiles((current) => ({ ...current, marksheet: value }))} /><FilePicker label="অভিভাবকের পেশার প্রমাণপত্র / চা-বাগানের অধিবাসী প্রত্যয়নপত্র আপলোড করো" value={files.proof} error={fieldErrors.proof} onChange={(value) => setFiles((current) => ({ ...current, proof: value }))} /></section>
      ...
      <section className="card program-plan"><h2>কর্মসূচির প্রক্রিয়া সংক্ষেপে</h2><ol><li><b>আবেদন জমা:</b> এই ফর্মটি পূরণ করে অনলাইনে আবেদন করতে হবে।</li><li><b>তথ্য যাচাই:</b> জমা দেওয়া SSC মার্কশীট, অভিভাবকের পেশা/আয়ের প্রমাণ এবং চা-বাগানের অধিবাসী প্রত্যয়নপত্র যাচাই করা হবে—পরিবারটি আর্থিকভাবে অসচ্ছল কি না এবং শিক্ষার্থীর উচ্চশিক্ষার প্রতি আগ্রহ আছে কি না দেখা হবে।</li><li><b>শর্টলিস্টিং ও ভাইভা:</b> যাচাই-বাছাইয়ে যোগ্য বিবেচিত আবেদনকারীদের সংক্ষিপ্ত তালিকা তৈরি করে মৌখিক পরীক্ষায় অংশ নিতে হবে।</li><li><b>চূড়ান্ত নির্বাচন ও বই সহায়তা:</b> পারিবারিক আর্থিক অবস্থা, ভাইভার ফলাফল ও উচ্চশিক্ষার প্রতি আগ্রহ বিবেচনা করে যোগ্য শিক্ষার্থীদের চূড়ান্তভাবে নির্বাচন করা হবে। নির্বাচিতদের কাঙ্ক্ষিত পাঠ্যবই কিনে দেওয়া হবে।</li></ol><h2>সকল আবেদনকারীর জন্য দীর্ঘমেয়াদি পরিকল্পনা</h2><p>বই-সংক্রান্ত সহায়তার জন্য কোনো শিক্ষার্থী নির্বাচিত হোক বা না হোক, প্রত্যেক আবেদনকারী শিক্ষার্থীকে একাদশ-দ্বাদশ শ্রেণিকালীন বিভিন্ন শিক্ষা সহায়তা ও উন্নয়নমূলক কার্যক্রমের আওতায় আনার সম্ভাবনা নিয়ে আলোচনা চলছে।</p><ul><li>নিয়মিত দিকনির্দেশনা</li><li>ফলাফলের ভিত্তিতে আর্থিক সহযোগিতা</li><li>প্রশিক্ষিত শিক্ষকদের মাধ্যমে বাংলা, ইংরেজি ও সাধারণ জ্ঞানের বিনামূল্যে মাসিক ক্লাস, প্রতি মাসে পাঠদান ও পর্যায়ক্রমে মূল্যায়ন</li></ul></section><footer><button className="admin-link" aria-label="প্রশাসন প্যানেল" onClick={() => setShowAdmin((value) => !value)}><LockKeyhole size={16} /></button></footer>
